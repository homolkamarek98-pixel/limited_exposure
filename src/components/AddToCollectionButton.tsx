...
export default function AddToCollectionButton({ editionId, soldOut, expired }: Props) {
      ...
      <div className="w-full space-y-3">
        ...
        <div className="flex gap-0">
          ...
          <button
            ...
            className="bg-mocha text-on-primary px-6 font-label text-xs uppercase tracking-widest font-bold hover:opacity-90 transition-opacity"
          >
            OK
          </button>
        </div>
  ...
  return (
    <button
      onClick={handleClick}
      className="w-full bg-mocha text-on-primary py-5 px-12 font-label text-xs uppercase tracking-[0.2em] font-bold hover:bg-primary-container transition-all"
    >
      Přidat do kolekce
    </button>
  );
